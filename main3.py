...
class GlobalDiscriminator(nn.Module):
    def __init__(self, c0_in, l0_in, l1_in):
        ...
        self.l0 = nn.Linear(l0_in, 16)  # 修改这里以满足合适的输入输出大小
...
def train_epoch(train_loader, generator, discriminator, optimizer_G, optimizer_D, device):
    g_loss = 0.0
    d_loss = 0.0
    ...
    for i, train_data in enumerate(train_loader):
        ...
        feature_map, shuffle_feature_map = shuffle_tensor(feature_map_list[3])
        G_Loss = autoencoder_loss(train_data, recon) - 0.1 * (
                    generator_loss(discriminator(train_data), true_label) + generator_loss(discriminator(recon.detach()),
                                                                                           false_label)) / 2 + infomax_loss(
...
def valid_epoch(test_loader, generator, discriminator, device):
    ...
    for j, test_data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            recon = generator.decoder(latent_code)
            shuffle_feature_map = shuffle_tensor(feature_map_list[8])

            test_Loss = autoencoder_loss(test_data, recon) + infomax_loss(latent_code, feature_map_list[8], shuffle_feature_map)
...
if __name__ == '__main__':
    ...
    for train_index, test_index in kf.split(raw_data):
        ...
        for epoch in range(epochs):
            g_loss, d_loss = train_epoch(train_loader, generator, discriminator, optimizer_G, optimizer_D, device)
            test_loss, z = valid_epoch(test_loader, generator, discriminator, device)
            ...
            # Save model
            # if epoch == (epochs - 1):
            #     os.chdir('D:\\huangyuwen\\code\\python\\dual_EEGFuseNet\\毕设\\(未调权重)ae_gan_mine_deap')
            #     torch.save(generator.state_dict(), f'v2generator_{sub}.pkl')
            #     torch.save(discriminator.state_dict(), f'v2discriminator_{sub}.pkl')
            #     print("feature:" + str(np.shape(z)))
            ...
            # writer.add_scalar('Loss/Critic', d_loss, epoch)
            # writer.add_scalar('Loss/test', test_loss, epoch)
            # TensorBoard_model(generator, critic, epoch)
            log_dir = os.path.join('./logs', f'{sub}_MIlog')
        ...
        path = os.path.join('./checkpoints',
                            f'MSE+Dis_sub{sub}_session1_{now}.pkl')
