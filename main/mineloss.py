...
class MINE(nn.Module):
    def __init__(self, y, M_P, y_M, alpha=1.0, beta=1.0, G_maet=0, L_maet=0 ):
        super().__init__()
    ...
    def forward(self, y, M, M_prime):
        ...
        # M_prime: 随机打乱的局部特征图，shape: (128, 1, 16, 96)
        # 计算全局损失
        G_pos_scores = self.global_d(y, M)  # 正样本的全局损失
        G_neg_scores = self.global_d(y, M_prime)  # 负样本的全局损失
        GLOBAL, self.G_ma_et = mine2_loss(G_pos_scores, G_neg_scores, self.G_ma_et)
        GLOBAL = GLOBAL*self.alpha  # 总全局损失
        ...
        y_M_prime = torch.cat((M_prime, y_exp), dim=3)  # shape: (128, 1, 16, 96+192)
        # 计算局部损失
        L_pos_scores = self.local_d(y_M)  # 正样本的局部损失
        L_neg_scores = self.local_d(y_M_prime)  # 负样本的局部损失
        LOCAL, self.L_ma_et = mine2_loss(L_pos_scores, L_neg_scores, self.L_ma_et)
        LOCAL = LOCAL*self.beta  # 总局部损失
        # 返回总损失（局部损失 + 全局损失）
        return LOCAL + GLOBAL
...
def pretrain(miloss, test_loader, load_pretrained=False):
    ...
    if miloss == 1:
        ...
        mi_loss = MINE(latent_code, M_P, y_M).to(device).float()
    ...
    optimizer_mi = torch.optim.Adam(mi_loss.parameters(), lr=0.0001)
...
def train_epoch(train_loader, generator, discriminator, optimizer_G, optimizer_D, device, num):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        D_Loss = 0.1 * (discriminator_loss(discriminator(train_data.detach()), true_label) + discriminator_loss(
        ...
        G_Loss = autoencoder_loss(train_data, recon) + mi_loss(latent_code, feature_map, shuffle_feature_map)
        g_loss += G_Loss.item()
    ...
    for train_index, test_index in kf.split(raw_data):
        ...
        for num in range(11):
            ...
            generator, discriminator, mi_loss, optimizer_G, optimizer_D, optimizer_mi = pretrain(
                2, test_loader, load_pretrained=False)
            ...
            path = os.path.join('../checkpoints', file_name,
                                f'mine_sub{sub}_featuremap{num}_{now}.pkl')
