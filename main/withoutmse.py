...
def get_data():
    ...
    # folder_path = 'D:\\suntuo\\EEGFuseNet_Daul_MI\\data\\2_preprocessed_byJANE'
    # 初始化一个空的列表来存储所有矩阵
    all_matrices = []
    # 遍历文件夹中的所有MAT文件
...
class LinearSimilarityDiscriminator(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(input.size(2)):
            x = input[:, :, i, :]
            x = x.view(x.size(0), -1)  # Flatten (128, 1, 1, 288) -> (128, 288)
            x = F.relu(self.fc1(x))
            score = self.fc2(x)
            scores.append(score)
...
class DeepInfoMaxLoss(nn.Module):
    def __init__(self, y, M_P, y_M, alpha=1.0, beta=1.0):
        super().__init__()

        self.alpha = alpha
        self.beta = beta
        self.global_d = GlobalDiscriminator(M_P.shape[1], 32 * (M_P.shape[3]), 128 + y.shape[1]).to(device)
        self.local_d = LinearSimilarityDiscriminator(y_M.shape[3]).to(device)
    # def GlobalDiscriminator(self, y, M):
    #         h = F.relu(self.c0(M))  # 修改输入为 M
    #         h = self.c1(h)
    #         h = h.view(y.shape[0], -1)
    #         h = F.relu(self.l0(h))
    ...
    def forward(self, y, M, M_prime):
        ...
        # y_exp = y_exp.expand(-1, -1, 1, M.shape[3])  # shape: (128, 192, 1, 96)
        ...
        M_prime = M[torch.randperm(M.shape[0]), :, :, :]
        M = M.permute(0, 2, 1, 3)
...
class MINE(nn.Module):
    ...
    def forward(self, y, M, M_prime):
        ...
        # y_M = torch.cat((M, y_exp), dim=1)  # shape: (128, 16+192, 1, 96)
        # y_M_prime = torch.cat((M_prime, y_exp), dim=1)  # shape: (128, 16+192, 1, 96)
        self.global_d = GlobalDiscriminator(M.shape[1], 32 * (M.shape[3]), 16 + y.shape[1]).to(device)
        # 计算全局损失
        Ej = -F.softplus(-self.global_d(y, M)).mean()  # 正样本的全局损失
        Em = F.softplus(self.global_d(y, M_prime)).mean()  # 负样本的全局损失
        GLOBAL = (Em - Ej) * self.alpha  # 总全局损失
...
def pretrain(miloss, test_loader, load_pretrained=False):
    """
    ...
    bs_data = Variable(next(iter(test_loader)).to(device))
    latent_code, feature_map1, feature_map_list = generator(bs_data)
    M_P = feature_map_list[num]
    M = feature_map_list[num].permute(0, 2, 1, 3)
    y_exp = latent_code.unsqueeze(1).unsqueeze(2)
    y_exp = y_exp.expand(-1, -1, M.shape[2], -1)
    # 将 y_exp 与 M 和 M_prime 进行拼接
    y_M = torch.cat((M, y_exp), dim=3)
    if miloss == 1:
        mi_loss = DeepInfoMaxLoss(latent_code, M_P, y_M).to(device).float()
    else:
        mi_loss = MINE().to(device).float()

    file_path = __file__
    file_name = os.path.basename(file_path)
    log_dir = os.path.join('../logs', file_name, f'{num}_MIlog')
    ...
    optimizer_mi = torch.optim.Adam(mi_loss.parameters(), lr=0.01)

    return generator, discriminator, mi_loss, optimizer_G, optimizer_D, optimizer_mi
...
def train_epoch(train_loader, generator, discriminator, optimizer_G, optimizer_D, device, num, dis):
    g_loss = 0.0
    d_loss = 0.0
    ...
    for i, train_data in enumerate(train_loader):
        ...
        # g_Loss = autoencoder_loss(train_data, recon)
        mi_Loss = mi_loss(latent_code, feature_map, shuffle_feature_map)
        G_Loss = mi_Loss
        print(mi_Loss)
        g_loss += G_Loss.item()
        G_Loss.backward()
...
def valid_epoch(test_loader, generator, discriminator, device, num):
    ...
    for j, test_data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            recon = generator.decoder(latent_code)
            feature_map, shuffle_feature_map = shuffle_tensor(feature_map_list[num])
            # test_Loss = autoencoder_loss(test_data, recon) + mi_loss(latent_code, feature_map, shuffle_feature_map)
            test_Loss = mi_loss(latent_code, feature_map, shuffle_feature_map)
...
if __name__ == '__main__':
    ...
    for train_index, test_index in kf.split(raw_data):
        ...
        for num in range(11):
            ...
            generator, discriminator, mi_loss, optimizer_G, optimizer_D, optimizer_mi = pretrain(
                1, test_loader, load_pretrained=False)

            # for epoch in range(1):
            for epoch in range(epochs):
                g_loss, d_loss = train_epoch(train_loader, generator, discriminator, optimizer_G, optimizer_D, device, num, dis=(epoch+1) % 3 == 0)
